...
use cosmwasm_std::{
    coin, from_binary, to_binary, Addr, Binary, CanonicalAddr, CosmosMsg, Decimal, Decimal256, Deps,
    DepsMut, Env, MessageInfo, Reply, ReplyOn, Response, StdError, StdResult, SubMsg, Uint128,
    Uint256, WasmMsg, QueryRequest, WasmQuery, QuerierWrapper,
};

use classic_bindings::{TerraMsg, TerraQuery};

use classic_dfluncswap::asset::{Asset, AssetInfo, PairInfo, PairInfoRaw};
use classic_dfluncswap::pair::{
    Cw20HookMsg, ExecuteMsg, InstantiateMsg, MigrateMsg, PoolResponse, QueryMsg,
    ReverseSimulationResponse, SimulationResponse,
};
use classic_dfluncswap::lp_staker::{ ExecuteMsg as LpStakerExecuteMsg, QueryMsg as LpStakerQueryMsg };

use classic_dfluncswap::querier::query_token_info;
use classic_dfluncswap::token::InstantiateMsg as TokenInstantiateMsg;
use classic_dfluncswap::util::{assert_deadline, migrate_version};
...
pub fn instantiate(
    ...
    let pair_info: &PairInfoRaw = &PairInfoRaw {
        ...
        lp_stake_contract: deps.api.addr_canonicalize(&msg.lp_stake_contract)?,
...
pub fn swap(
    ...
    match ask_pool.info.clone() {
        AssetInfo::NativeToken { denom } => {
            let mut msgs: Vec<CosmosMsg<TerraMsg>> = distribute_fee(&deps.querier.clone(),
                deps.api.addr_humanize(&pair_info.lp_stake_contract)?.to_string(),
                commission_amount,
                pair_info.fee_rate,
        ...
        AssetInfo::Token { contract_addr } => {
            let mut msgs = distribute_fee(&deps.querier.clone(),
                deps.api.addr_humanize(&pair_info.lp_stake_contract)?.to_string(),
                commission_amount,
                pair_info.fee_rate,
                None,
...
fn distribute_fee(
    ...
    lp_stake_contract: String,
    commission_amount: Uint128,
    fee_rate_to_lp_staker: u64,
    native_token_symbol: Option<String>,
    cw20_address: Option<String>,
) -> StdResult<Vec<CosmosMsg<TerraMsg>>> {    
    let mut messages: Vec<CosmosMsg<TerraMsg>> = vec![];

    let fee_to_lp_staker = commission_amount * Uint128::from(fee_rate_to_lp_staker) / Uint128::from(10000u128);
    
    // native token as swap fee
    if let Some(native_token_symbol) = native_token_symbol {
        let deposit_native_token_fee_to_lp_staker_msg: CosmosMsg<TerraMsg> = CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: lp_stake_contract.clone(),
            msg: to_binary(&LpStakerExecuteMsg::Deposit {
            })?,
            funds: vec![coin(fee_to_lp_staker.u128(), native_token_symbol)],
        });
        messages.push(deposit_native_token_fee_to_lp_staker_msg);  
    } else { // cw20 token as swap fee
        let cw20_address = cw20_address.expect("CW20 address must be provided.");
        let is_valid_cw20 = querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
            contract_addr: lp_stake_contract.clone(),
            msg: to_binary(&LpStakerQueryMsg::IsDividendCw20 { cw20_address: cw20_address.clone() })?,
        }))?;
        if is_valid_cw20 {
            messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: cw20_address.clone(),
                msg: to_binary(&Cw20ExecuteMsg::IncreaseAllowance {
                    spender: lp_stake_contract.clone(),
                    amount: fee_to_lp_staker,
                    expires: None,
                })?,
                funds: vec![],
            }));

            let deposit_cw20_fee_to_lp_staker_msg: CosmosMsg<TerraMsg> = CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: lp_stake_contract.clone(),
                msg: to_binary(&LpStakerExecuteMsg::DepositCw20 { 
                    cw20_address, 
                    amount: fee_to_lp_staker,
                })?,
                funds: vec![],
            });
            messages.push(deposit_cw20_fee_to_lp_staker_msg);  
        }
    }
